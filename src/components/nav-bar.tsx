        ...
        {(["lighting", "elevator"] as const).map((t) => {
          ...
          return (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`py-3 rounded-2xl transition-all duration-200 active:scale-95 ${active ? "bg-linear-to-br from-lol-pink to-lol-purple shadow-[0_0_20px_0_rgba(255,62,181,0.4)] " : "bg-white/5  border border-solid border-white/10"}`}
            >
              <p
